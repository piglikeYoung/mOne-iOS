...
@interface JHThingViewController ()<iCarouselDataSource, iCarouselDelegate>
...
@implementation JHThingViewController
...
- (void)requestThingContentAtIndex:(NSInteger)index {
    ...
    JHThingParam *param = [JHThingParam param];
    param.index = [NSNumber numberWithInteger:index];
    [JHThingTool thingContentWithParam:param success:^(JHThingResult *result) {
        if ([result.rs isEqualToString:kSuccessFlag]) {
            ...
            if (self.isRefreshing) {
                ...
                if (self.items.count > 0) {
                    ...
                    if ([result.entTg.strId isEqualToString:((JHThingInfo *)self.items[0]).strId]) {
                    ...
                    // 不相同，放到数组里
                        ...
                        dispatch_once(&onceToken, ^{
                            [self requestThingContentAtIndex:self.items.count];
                        });
                    }
                }
                // 之前没数据
            ...
            // 左拉加载
                ...
                // 添加数据到数组中
                [self.items addObject:result.entTg];
                // 将新的item插入末尾
                [self.carousel insertItemAtIndex:self.items.count animated:YES];
                // 显示label
                self.leftRefreshLabel.hidden = NO;
...
- (UIView *)carousel:(iCarousel *)carousel viewForItemAtIndex:(NSInteger)index reusingView:(UIView *)view {
    ...
    JHThingView *thingView = (JHThingView *)view;
    
    // 复用View
    if (!thingView) {
        thingView = [[JHThingView alloc] initWithFrame:carousel.bounds];
    }
    thingView.thingInfo = self.items[index];
