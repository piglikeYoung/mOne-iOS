...
@interface JHReadingViewController ()<iCarouselDataSource, iCarouselDelegate>
...
@implementation JHReadingViewController
...
- (void)requestReadingContentAtIndex:(NSInteger)index {
    ...
    JHReadingParam *param = [JHReadingParam param];
    param.index = [NSNumber numberWithInteger:index];
    [JHReadingTool readingContentWithParam:param success:^(JHReadingResult *result) {
        if ([result.result isEqualToString:kSuccessFlag]) {
            ...
            if (self.isRefreshing) {
                ...
                if (self.items.count > 0) {
                    ...
                    if ([result.contentEntity.strContentId isEqualToString:((JHReadingInfo *)self.items[0]).strContentId]) {
                    ...
                    // 不相同，放到数组里
                        ...
                        // 添加到数组
                        [_items addObject:result.contentEntity];
                        // 将新的item插入末尾
                        ...
                        dispatch_once(&onceToken, ^{
                            [self requestReadingContentAtIndex:self.items.count];
                        });
                    }
                }
                // 之前没数据
            ...
            // 左拉加载
                ...
                // 添加数据到数组中
                [self.items addObject:result.contentEntity];
                // 将新的item插入末尾
                [self.carousel insertItemAtIndex:self.items.count animated:YES];
                // 显示label
                self.leftRefreshLabel.hidden = NO;
...
- (UIView *)carousel:(iCarousel *)carousel viewForItemAtIndex:(NSInteger)index reusingView:(UIView *)view {
    ...
    // 复用View
    if (!readingView) {
        readingView = [[JHReadingView alloc] initWithFrame:carousel.bounds];
    }
    readingView.readingInfo = self.items[index];
