...
@interface JHQuestionViewController ()<iCarouselDataSource, iCarouselDelegate>
...
@implementation JHQuestionViewController
...
- (void)requestQuestionContentAtIndex:(NSInteger)index {
    ...
    JHQuestionParam *param = [JHQuestionParam param];
    param.index = [NSNumber numberWithInteger:index];
    [JHQuestionTool questionContentWithParam:param success:^(JHQuestionResult *result) {
        if ([result.result isEqualToString:kSuccessFlag]) {
            ...
            if (self.isRefreshing) {
                ...
                if (self.items.count > 0) {
                    ...
                    if ([result.questionAdEntity.strQuestionId isEqualToString:((JHQuestionInfo *)self.items[0]).strQuestionId]) {
                    ...
                    // 不相同，放到数组里
                        ...
                        // 添加到数组
                        [_items addObject:result.questionAdEntity];
                        // 将新的item插入末尾
                        ...
                        dispatch_once(&onceToken, ^{
                            [self requestQuestionContentAtIndex:self.items.count];
                        });
                    }
                }
                // 之前没数据
            ...
            // 左拉加载
                ...
                // 添加数据到数组中
                [self.items addObject:result.questionAdEntity];
                // 将新的item插入末尾
                [self.carousel insertItemAtIndex:self.items.count animated:YES];
                // 显示label
                self.leftRefreshLabel.hidden = NO;
...
- (UIView *)carousel:(iCarousel *)carousel viewForItemAtIndex:(NSInteger)index reusingView:(UIView *)view {
    ...
    JHQuestionView *questionView = (JHQuestionView *)view;
    
    // 复用View
    if (!questionView) {
        questionView = [[JHQuestionView alloc] initWithFrame:carousel.bounds];
    }
    questionView.questionInfo = self.items[index];
